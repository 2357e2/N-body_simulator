...
ymax = 8
particleMass = 1e-5
simulationLength = 100 # update() pause parameter
...
rotationDirection = 1 # 1 for CCW, -1 for CW
energy = 0 # Energy of perturbing galaxy's orbit. 0 for parabolic orbit
focus = 8 # distance of closest approach
perturbationMass = 2
...
def create_galaxy (position, velocity, mass):
    ...
    for i in range(5):
        particleList = []
        radius = 2+i
        for j in range(12 + 6*i): # rings have 12, 18, 24, 30, 36 particles
            angle = 2*np.pi*j/(12 + 6*i)
            if (angle > np.pi):
                angle -= 2*np.pi # Angle range -pi to +pi
            particle = Particle(j, particleMass, radius, angle, static_velocity(radius, mass), i, 0)
            particleList.append(particle)
        ring = Ring(i, radius, particleList) # rings have radii of 2,3,4,5,6
